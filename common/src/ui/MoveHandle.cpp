...
double MoveHandle::handleLength(const render::Camera& camera) const
{
...
vm::segment3d MoveHandle::axisSegment(
...
mdl::Hit MoveHandle::pick3D(const vm::ray3d& pickRay, const render::Camera& camera) const
{
  ...
  const HitArea area) const
{
  ...
  color = color.mixed(Color{1, 1, 1, color.a()}, kHighlightMix);
