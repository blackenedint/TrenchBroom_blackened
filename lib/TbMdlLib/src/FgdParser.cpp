...
Result<PropertyValueTypes::IOParameterType> parseIOParameterType(
  const std::string_view typeName)
{
  using PropertyValueTypes::IOParameterType;
...
DecalDefinition FgdParser::parseDecal()
{
  m_tokenizer.nextToken(FgdToken::OParenthesis);
  ...
  if (token.hasType(FgdToken::CParenthesis))
  {
    ...
    return DecalDefinition{std::move(defaultExp)};
  }
  ...
  return parseElModelExpression(m_tokenizer, location, FgdToken::CParenthesis)
         ...
         | kdl::transform(
           [](auto expression) { return DecalDefinition{std::move(expression)}; })
         | kdl::if_error([](const auto& e) { throw ParserException{e.msg}; })
         | kdl::value();
}
...
PropertyDefinition FgdParser::parseTargetDestinationPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
...
PropertyDefinition FgdParser::parseStringPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
...
PropertyDefinition FgdParser::parseIntegerPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Integer{std::move(defaultValue)},
    std::move(shortDescription),
    std::move(longDescription),
    readOnly};
}

PropertyDefinition FgdParser::parseFloatPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Float{std::move(defaultValue)},
    std::move(shortDescription),
    std::move(longDescription),
    readOnly};
}

PropertyDefinition FgdParser::parseChoicesPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
  ...
  auto options = std::vector<PropertyValueTypes::ChoiceOption>{};
  while (token.type() != FgdToken::CBracket)
  {
    ...
    options.push_back(
      PropertyValueTypes::ChoiceOption{std::move(value), std::move(caption)});
    token = m_tokenizer.nextToken(
      FgdToken::Integer | FgdToken::Decimal | FgdToken::String | FgdToken::CBracket);
  }
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Choice{std::move(options), std::move(defaultValue)},
    std::move(shortDescription),
    std::move(longDescription),
    readOnly};
}

PropertyDefinition FgdParser::parseFlagsPropertyDefinition(std::string propertyKey)
{
  ...
  auto flags = std::vector<PropertyValueTypes::Flag>{};
  auto defaultValue = 0;
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Flags{std::move(flags), defaultValue},
    "",
    "",
    false};
}

PropertyDefinition FgdParser::parseOriginPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Origin{std::move(defaultValue)},
    std::move(shortDescription),
    std::move(longDescription),
    readOnly};
}

PropertyDefinition FgdParser::parseInputPropertyDefinition(ParserStatus& status)
{
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Input{parameterType},
    std::move(description),
    {},
    false};
}

PropertyDefinition FgdParser::parseOutputPropertyDefinition(ParserStatus& status)
{
  auto [propertyKey, parameterType] = parseIOProperty(status);
  auto description = parsePropertyDescription();
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Output{parameterType},
    std::move(description),
    {},
    false};
}

PropertyDefinition FgdParser::parseColorPropertyDefinition(
  ParserStatus& status, const ColorType colorType, std::string propertyKey)
{
  ...
  case ColorType::Color1:
    return {
      std::move(propertyKey),
      PropertyValueTypes::Color<RgbF>{std::move(defaultValue)},
      std::move(shortDescription),
      std::move(longDescription),
      readOnly};
  case ColorType::Color255:
    return {
      std::move(propertyKey),
      PropertyValueTypes::Color<RgbB>{std::move(defaultValue)},
      std::move(shortDescription),
      std::move(longDescription),
      readOnly};
    switchDefault();
...
PropertyDefinition FgdParser::parseUnknownPropertyDefinition(
  ParserStatus& status, std::string propertyKey)
{
  const auto readOnly = parseReadOnlyFlag(status);
  auto shortDescription = parsePropertyDescription();
  ...
  return {
    std::move(propertyKey),
    PropertyValueTypes::Unknown{std::move(defaultValue)},
    std::move(shortDescription),
    std::move(longDescription),
    readOnly};
}
...
std::tuple<std::string, PropertyValueTypes::IOParameterType> FgdParser::parseIOProperty(
  ParserStatus&)
{
